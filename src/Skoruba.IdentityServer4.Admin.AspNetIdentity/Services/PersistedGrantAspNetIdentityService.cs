...
using Skoruba.IdentityServer4.Admin.AspNetIdentity.Mappers;
...
namespace Skoruba.IdentityServer4.Admin.AspNetIdentity.Services
{
        ...
        where TUserToken : IdentityUserToken<TKey>
    {
        ...
        public async Task<PersistedGrantsDto> GetPersitedGrantsByUser(string subjectId, int page = 1, int pageSize = 10)
        {
            ...
            if (!exists) throw new UserFriendlyErrorPageIdentityException(string.Format(_persistedGrantAspNetIdentityServiceResources.PersistedGrantWithSubjectIdDoesNotExist().Description, subjectId), _persistedGrantAspNetIdentityServiceResources.PersistedGrantWithSubjectIdDoesNotExist().Description);
        ...
        public async Task<PersistedGrantDto> GetPersitedGrantAsync(string key)
        {
            ...
            if (persistedGrant == null) throw new UserFriendlyErrorPageIdentityException(string.Format(_persistedGrantAspNetIdentityServiceResources.PersistedGrantDoesNotExist().Description, key), _persistedGrantAspNetIdentityServiceResources.PersistedGrantDoesNotExist().Description);
